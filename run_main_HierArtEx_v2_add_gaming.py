...
if False:
  for vn in GENERALIST_FEAT_SIZE_DICT.keys():
    print("="*10, vn)
    if vn == "clip":
        print(torch.load(f"./dataset_gaming_VR/filtered_video_game_features/ViT-B-32_laion2b_s34b_b79k/unpacked_images/3t_PvlmUpyY.pt", weights_only=True).shape, torch.load(f"./dataset_gaming_VR/filtered_video_game_features/ViT-B-32_laion2b_s34b_b79k/unpacked_images/3t_PvlmUpyY.pt", weights_only=True).dtype)
        print(torch.load(f"./dataset_gaming_VR/filtered_video_game_features/ViT-B-32_laion2b_s34b_b79k/descriptions/sentences/3t_PvlmUpyY.pt", weights_only=True).shape, torch.load(f"./dataset_gaming_VR/filtered_video_game_features/ViT-B-32_laion2b_s34b_b79k/descriptions/sentences/3t_PvlmUpyY.pt", weights_only=True).dtype)
        print(len(pd.read_pickle(f"./dataset_gaming_VR/filtered_video_game_features/ViT-B-32_laion2b_s34b_b79k/descriptions/tokens_strings/3t_PvlmUpyY.pkl")))
    else:
        try:
            print(torch.load(f"./dataset_gaming_VR/filtered_video_game_features/{vn}/unpacked_images/3t_PvlmUpyY.pt", weights_only=True).shape, torch.load(f"./dataset_gaming_VR/filtered_video_game_features/{vn}/unpacked_images/3t_PvlmUpyY.pt", weights_only=True).dtype)
            print(torch.load(f"./dataset_gaming_VR/filtered_video_game_features/{vn}/descriptions/sentences/3t_PvlmUpyY.pt", weights_only=True).shape, torch.load(f"./dataset_gaming_VR/filtered_video_game_features/{vn}/descriptions/sentences/3t_PvlmUpyY.pt", weights_only=True).dtype)
            print(len(pd.read_pickle(f"./dataset_gaming_VR/filtered_video_game_features/{vn}/descriptions/tokens_strings/3t_PvlmUpyY.pkl")))
        except:
...
def main_proc(_loader, _model_desc_pov, _model_pov, _phase, _indices, _eval, _scheduler, _loss_fn, _loss_fn_room, _optimizer, _room_loss_weight):
    ...
    with torch.set_grad_enabled(_phase == 'train'):
        for i, batch_data in enumerate(_loader):
            ...
            #print(output_desc_pov.shape, output_pov.shape, 
            #      room_len_pov,
            #      output_room_lev_desc.shape if output_room_lev_desc is not None else None, 
            #      tmp_output_pov_room_level.shape if tmp_output_pov_room_level is not None else None)
            #if output_room_lev_desc.shape != tmp_output_pov_room_level.shape:
            #    print("SHAPE MISMATCH", names, output_room_lev_desc.shape, tmp_output_pov_room_level.shape)
            ...
            # @DONE add loss for room processing (matrix of txt/vis vector per room)
            # @DONE add wandb anchors for this
            # @TODO these are likely to be done during val-train/val/test
...
if __name__ == "__main__":
    ...
    """train_dataset = DescriptionSceneMuseum(f"./{base_path}/descriptions/sentences", 
                                    ...
                                    {"train": np.arange(239)}, "train", num_imgs=args.n_imgs)
    ...
    for n_try in range(number_of_tries):
        ...
        model_pov.eval()
        model_desc_pov.eval()
        output_description_test, output_pov_test = main_proc(test_loader, model_desc_pov, model_pov, "test", indices_game, True, scheduler, loss_fn, loss_fn_room, optimizer, room_loss_weight)
