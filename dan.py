...
class DAN(nn.Module):
    ...
    def __init__(self, h, feature_dim, num_heads, inner_dim, dropout, output_feature_size, art_features_size=0):
        ...
        if art_features_size != 0:
            self.d_view_feature += art_features_size
        ...
        self.classifier = nn.Linear(self.d_view_feature, output_feature_size)

    def forward(self, batch_size, max_num_views, num_views, x, use_utilization=False):
        #y = self.extractor(x)
        # y received as (bsz, n_views, in_channels)
        y = x
        k = []
        u = []
        count = 0
